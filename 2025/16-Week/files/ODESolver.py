"""
Adapted code from https://github.com/sundnes/solving_odes_in_python
...
class Heun(ODESolver):
    ...
    u_hat = u[n] + dt * f(t[n], u[n])
    u[n+1] = u[n] + dt/2 * (f(t[n], u[n]) + f(t[n]+dt, u_hat))
    """
    def advance(self):
        u, f, n, t = self.u, self.f, self.n, self.t
        dt = self.dt
...
class ExplicitMidpoint(ODESolver):
    """
    Explicit Midpoint method for ODE solving.
    u_mid = u[n] + dt/2 * f(t[n], u[n])
    u[n+1] = u[n] + dt * f(t[n] + dt/2, u_mid)
    """
    def advance(self):
        u, f, n, t = self.u, self.f, self.n, self.t
        dt = self.dt
...
class RungeKutta4(ODESolver):
    """
    Classical Runge-Kutta method (RK4) for ODE solving.
...
def test_exact_numerical_solution():
    ...
    for this choice.
    """
    solver_classes = [ForwardEuler, Heun, ExplicitMidpoint, RungeKutta4]
    a = 0.2
    b = 3
    ...
    print("All methods passed the exact solution test.")
